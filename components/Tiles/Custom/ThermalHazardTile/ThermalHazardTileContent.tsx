...
export default function ThermalHazardTileContent({
  ...
  const has_perceived_temp =
    status_temp === 'success' && perceived_temperature !== null
  ...
  useEffect(() => {
    ...
    const hazard = harzard_data[day_index] as any
    const sign =
      has_perceived_temp && perceived_temperature![day_index] < thermal_limit
        ? -1
        : 1
    setHazardIndex(hazard * sign)
  }, [perceived_temperature, harzard_data, day_index, has_perceived_temp])

  const temp_settled = status_temp === 'success' || status_temp === 'error'
  if (!harzard_data || status_data !== 'success' || !temp_settled) {
    return <RequestIndicator failed={status_data === 'error'} />
  }
