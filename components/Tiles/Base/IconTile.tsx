...
function getDownloadUrl(
  ...
  if (datasources && datasources.length > 0) {
    ...
    const allowed = datasources.find(ds => ds.allow_download && ds.file_name)
    if (allowed && allowed.file_name) {
      return `/api/download/${allowed.file_name}`
    }
  }
  return undefined
}
      ...
      <div className="mb-4 flex flex-col gap-2">
        <div className="relative flex items-stretch gap-4">
          <div className="flex flex-grow flex-wrap items-center justify-start gap-x-4">
            <Title
              as={'h2'}
              className={cx('min-w-fit mb-0', iconTileTitleStyle({ variant }))}
              font={'normal'}
            >
              {title ?? (
